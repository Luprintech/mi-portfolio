    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pt-32 pb-20 px-4 md:px-8 relative overflow-hidden selection:bg-cyan-500/30">
      ...
      >
        <div className="text-center mb-16">
          ...
          <p className="typo-body text-xl text-[var(--text-secondary)] font-medium mb-4">{t('contact.subtitle')}</p>
          <p className="typo-body max-w-2xl mx-auto text-[var(--text-muted)] leading-relaxed">{t('contact.intro')}</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-stretch min-h-[600px]">
          {/* Columna izquierda */}
          <motion.div
          ...
          >
            {/* Formas directas */}
            <div className="bg-[var(--bg-surface)] backdrop-blur-md p-6 md:p-8 rounded-2xl border border-[var(--border-color)] hover:border-[var(--accent-primary)]/40 transition-all duration-300 shadow-[var(--card-shadow)] flex-1 flex flex-col justify-between min-h-[260px]">
              <h3 className="typo-title text-2xl font-bold text-[var(--text-primary)] mb-6 flex items-center gap-3">
              ...
              <div className="space-y-6">
                <div className="group">
                  <label className="typo-label text-sm text-[var(--text-muted)] font-semibold uppercase tracking-wider mb-1 block">{t('contact.email_label')}</label>
                ...
                <div>
                  <label className="typo-label text-sm text-[var(--text-muted)] font-semibold uppercase tracking-wider mb-1 block">{t('contact.location_label')}</label>
            ...
            <div className="bg-[var(--bg-surface)] backdrop-blur-md p-6 md:p-8 rounded-2xl border border-[var(--border-color)] hover:border-[var(--accent-secondary)]/30 transition-all duration-300 shadow-[var(--card-shadow)] flex-1 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 flex-1 place-items-center text-center h-full min-h-[90px]">
                <a href="https://www.linkedin.com/in/guadalupe-cano-moyano/" target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="typo-label text-base font-medium text-[var(--text-secondary)] group-hover:text-white">LinkedIn</span>
                </a>
                <a href="https://github.com/Luprintech" target="_blank" rel="noopener noreferrer"
                  ...
                  <FaGithub className="text-5xl mb-2 text-[var(--text-muted)] group-hover:text-white transition-colors" />
                  <span className="typo-label text-base font-medium text-[var(--text-secondary)] group-hover:text-white">GitHub</span>
                </a>
                <a href="https://www.youtube.com/@Luprintech" target="_blank" rel="noopener noreferrer"
                  ...
                  <FaYoutube className="text-5xl mb-2 text-[var(--text-muted)] group-hover:text-white transition-colors" />
                  <span className="typo-label text-base font-medium text-[var(--text-secondary)] group-hover:text-white">YouTube</span>
                </a>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="bg-[var(--bg-surface)] backdrop-blur-md p-6 md:p-10 rounded-2xl border border-[var(--border-color)] shadow-[var(--shadow-md)] relative overflow-hidden flex-1 flex flex-col justify-between min-h-[460px]">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-fuchsia-500 via-purple-500 to-cyan-500" />

              <h3 className="typo-title text-2xl font-bold text-[var(--text-primary)] mb-8">{t('contact.form_title')}</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label htmlFor="name" className="typo-label text-sm font-medium text-[var(--text-muted)]">{t('contact.form_name')}</label>
                    <input type="text" id="name" name="name" required value={formData.name} onChange={handleChange}
                      className={inputClass} placeholder={t('contact.form_name_placeholder')} />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="email" className="typo-label text-sm font-medium text-[var(--text-muted)]">{t('contact.form_email')}</label>
                ...
                <div className="space-y-2">
                  <label htmlFor="subject" className="typo-label text-sm font-medium text-[var(--text-muted)]">{t('contact.form_subject')}</label>
                ...
                <div className="space-y-2">
                  <label htmlFor="message" className="typo-label text-sm font-medium text-[var(--text-muted)]">{t('contact.form_msg')}</label>
                ...
                <button
                  ...
                  className={`typo-label w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-all duration-300 ${
                    status === "success"
